"""Language-profile loader for the Coding Module.

A profile is a YAML file under ``coding-module/profiles/`` that owns all
language-specific conventions: file layout, sandbox image, tooling commands,
and LLM system prompts.  The core pipeline in ``src/nodes.py`` should not contain
any language-specific literals; it should consume values through a ``Profile``
instance.

The default profile is ``python``.  Requesting a profile that does not exist
falls back to the default so the service stays available even when an unknown
...
class Profile:
    ...
    def resolve(self, text: str) -> str:
        ...
        Placeholders such as ``__SOURCE_MAIN__`` are replaced by their concrete
        file paths.  This is used for both prompts and sandbox commands.
        """
        resolved = text
    ...
    def setup_files(self) -> Dict[str, str]:
        """Return the workspace setup files (e.g. conftest.py) as filename->content."""
...
def get_profile(name: str | None = None) -> Profile:
    ...
    The default profile is discovered from ``DEFAULT_PROFILE`` (``python``).  If
    the requested profile file does not exist, or if it is missing required keys,
    the default profile is loaded instead and a warning is printed to stderr.
    """
